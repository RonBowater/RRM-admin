...
const VAREDIT_TYPE_LONG_TEXT  = 3;
const VAREDIT_TYPE_EMAIL      = 4;
const VAREDIT_TYPE_DATE       = 5;
const VAREDIT_TYPE_PROT       = 6;
...
var VAREDIT_disable_field_types = false;
...
{ 
	...
	for (var ivar=0; ivar<VAREDIT_ctl.length; ivar++)
	{
		...
		labstr = VAREDIT_ctl[ivar][VAREDIT_DISPLAY_NAME].split("_").join(" ");

		if (VAREDIT_ctl[ivar][VAREDIT_TYPE]==VAREDIT_TYPE_YESNO)
		{ 
		    if (VAREDIT_disable_field_types==false)  
		    {
		    	labstr = labstr + " (Yes/No)";
		    }
			myobj = {name: thename, label: labstr, 
			control: "select", options: [{label: "Yes", value: 'Yes'}, {label: "No", value: 'No'}]};
		}
		else if (VAREDIT_ctl[ivar][VAREDIT_TYPE]==VAREDIT_TYPE_LONG_TEXT)
		{
			if (VAREDIT_disable_field_types==false)  
		    {
		    	labstr = labstr + " (Text)";
		...
		else if (VAREDIT_ctl[ivar][VAREDIT_TYPE]==VAREDIT_TYPE_EMAIL)
		{
			...
			var myobj = {name: thename, label: labstr, 
			control: "input"};
		}
		else if (VAREDIT_ctl[ivar][VAREDIT_TYPE]==VAREDIT_TYPE_DATE)
		{
			if (VAREDIT_disable_field_types==false)  
		    {
		    	labstr = labstr + " (Date)";
		...
		{
			alert ("Invalid varedit type " + VAREDIT_ctl[ivar][VAREDIT_TYPE]);
			return;
		}
	...
	for (var ivar=0; ivar<VAREDIT_ctl.length; ivar++)
	{
		...
		if (VAREDIT_ctl[ivar][VAREDIT_TYPE]==VAREDIT_TYPE_DATE)
		{
			...
			{
				val = mysql_to_dd_mname_yyyy (val);
			}
		}
		else
		{
